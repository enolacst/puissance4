...
class Board:
    ...
    def win(self) -> bool:
        ...
        if len(self.state) != 0: # si quelqu'un a joué

            if  self.timer < (self.stones * 2) - 1:
            # si pas assez de pierre ont été joué pour qu'il y ai une victoire
                #print('pas assez de stones jouées')
                return False
            else: 
               
                if self.opponent == 'J':
                    _state = self.state[::2]
                    #print('on vérifie pour J...')
                else: 
                    _state = self.state[1::2]
                    #print('on vérifie pour R...')
                #attribution :
                lastStone = self.state[-1]
                numLine = lastStone[0] #numéro ligne
                numCol = lastStone[1] #numéro colonne
                nbStone = 1 #compteur des pierres

                #cylindrique
                if (self.cylinder):
                    mod = self.nbc
                else:
                    mod = self.nbc+1 #modulo


                # on cherche les stones à droite
                #print('vérif à droite')
                count = 1
                while count < self.stones:
                    if (numLine, (numCol + count)%mod) in _state:
                        nbStone += 1
                    else:
                        count = self.stones
                    count += 1
                ...
                # on cherche les stones en haut
                #print('vérif en haut')
                count = 1
                while count < self.stones:
                    if ((numLine + count), numCol) in _state:
                        nbStone += 1
                    else:
                        count = self.stones
                    count += 1
                ...
                # on cherche les stones sur la diagonale en haut à gauche
                #print('vérif en haut à gauche')
                count = 1
                while count < self.stones:
                    if ((numLine + count), (numCol - count)%mod) in _state:
                        nbStone += 1
                    else:
                        count = self.stones
                    count += 1
                ...
                # on cherche les stones sur la diagonale en haut à droite
                #print('vérif en haut à droite')
                count = 1
                while count < self.stones:
                    if ((numLine + count), (numCol + count)%mod) in _state:
                        nbStone += 1
